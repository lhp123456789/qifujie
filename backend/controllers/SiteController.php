...
class SiteController extends \yii\web\Controller
{
    ...
    public function actionMain()
    {
        ...
        $temp = [
            'CATEGORY' => CategoryModel::find()->count('id'),
        ...
        $statics = [
            'CATEGORY' => [
                $temp['CATEGORY'],
                $temp['CATEGORY'] ? number_format(CategoryModel::find()->where([
                        'between',
                        'created_at',
                        strtotime(date('Y-m-01')),
                        strtotime(date('Y-m-01 23:59:59') . " +1 month -1 day")
                    ])->count('id') / $temp['CATEGORY'] * 100, 2) : $percent
            ],
            'COMMENT' => [
                $temp['COMMENT'],
            ...
            'USER' => [
                ...
                $temp['USER'] ? number_format(User::find()->where([
                        ...
                        strtotime(date('Y-m-01')),
                        strtotime(date('Y-m-01 23:59:59') . " +1 month -1 day")
                    ])->count('id') / $temp['USER'] * 100, 2) : $percent
            ],
            'FRIEND_LINK' => [
                $temp['FRIEND_LINK'],
