...
//            $fileType = strrchr($value1, '.');
    ...
    public function CompanyHot(){
	    $query = new Query();
	     $data = $query->select(['c1.id company_id','company_name','company_alias','logo','company_url','company_describe','hot','referrer'])
            ->from(['c1'=>$this->COMPANY_TABLE])
            ->leftJoin(['c3'=>$this->COMP_ADD_TABLE],'c3.comp_id=c1.id')
            ->where(['c1.status'=>1])
            ->andWhere(['c3.status'=>1])
            ->andWhere(['>','hot',1])
            ->andWhere(['>','referrer',1])
            ->orderBy('hot desc')
            ->all();
    ...
    /*按公司名称或类别模糊查询*/
    public function CompanyFuzzySearch($params=[]){
        $query = new Query();
        //页码
        ...
        //排序
        $params['order_by'] = 'c3.created_at asc';
        //序号
        $offset = ($this->defaultPage - 1) * $this->defaultPageSize;

        $query->select(['c3.id','c3.cid','c3.p_cid','c2.name type_name','c22.name P_type_name','company_name',
            'company_alias','logo','company_url','company_personner',
            'company_phone','company_product','company_describe'])
            ->from(['c1'=>$this->COMPANY_TABLE])
            ->leftJoin(['c3'=>$this->COMP_ADD_TABLE],'c3.comp_id=c1.id')
        ...
        if (!empty($params['keywords'])) {
            $query->andWhere(['or', ['like', 'company_name', $params['keywords']],
                ['like', 'company_alias', $params['keywords']],
                ['like','c2.name',$params['keywords']],
...
//        print_r($arr_data);exit;
    ...
    public function CompanyDetail($where=[]){
        $query = new Query();
        $query->select(['c3.id','c2.name','company_name','company_alias','logo','company_url','business_licese','company_personner','company_phone','cusc','id_number','company_product','company_describe','c1.uid','c1.id company_id','id_number','card_picture','card_pic','c1.status company_status','c3.short_name','c3.web_url web_url'])
            ->from(['c1'=>$this->COMPANY_TABLE])
            ->leftJoin(['c3'=>$this->COMP_ADD_TABLE],'c3.comp_id=c1.id')
            ->leftJoin(['c2'=>$this->CATEGORY_TABLE],'c2.id=c3.cid');
        if($where){
    ...
    public function CompanyTypeSearch($params=[]){
        ...
        $query->select(['c3.id','c2.name','company_name','company_alias','logo','web_url','company_personner',
                                'company_phone','company_product','company_describe','c22.name p_type_name','c22.id p_cid','c3.short_name','c1.id company_id'])
            ->from(['c1'=>$this->COMPANY_TABLE])
            ->leftJoin(['c3'=>$this->COMP_ADD_TABLE],'c3.comp_id=c1.id')
            ->leftJoin(['c2'=>$this->CATEGORY_TABLE],'c2.id=c3.cid')
            ->leftJoin(['c22'=>$this->CATEGORY_TABLE],'c22.id=c3.p_cid')
