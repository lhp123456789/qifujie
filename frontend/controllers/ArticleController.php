...
class ArticleController extends Controller
{
        ...
        return [
            [
                ...
                'lastModified' => function ($action, $params) {
                    ...
                    if( $model === null ) throw new NotFoundHttpException(Yii::t("frontend", "GoodsModel id {id} is not exists", ['id' => $id]));
   ...
   /* public function actionT(){
        $cate = Category::find()->andWhere(['parent_id' => 0])->asArray()->all();
        $tree = new FamilyTree($cate);
        $sss = $tree->getDescendants(0);
        var_dump($sss);exit;
    }*/
    /**
     * 分类列表页
    ...
    public function actionIndex($cat = '')
    {
        ...
        $cates = Category::find()->andWhere(['parent_id' => 0])->asArray()->all();
//        $path = Category::find()->where('perent_id',0) -> pluck('path');
//        var_dump($path);exit();
//        foreach($path as $k => $v){
//            $cids = Category::find()->Where('like','path',$v.",%",false) -> pluck('id') -> asArray();
//            $classifylist[] = GoodsModel::find()
//                ->select(['id','cid','title','thumb'])
//                ->Where('in','cid',$cids)
...
//
//        }
        $tree = new FamilyTree($cates);
        $classify = $tree->getDescendants(0);
        foreach ($classify as $k=>$v){
            $catelist[]=$classify[$k]['name'] ;
        }
//        var_dump($catelist);exit();
        return $this->render('index', compact('catelist'));
    }
